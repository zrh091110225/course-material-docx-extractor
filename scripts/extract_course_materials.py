...
COZE_FOOTER_RE = re.compile(r"^本内容由\s*Coze\s*AI\s*生成")
...
规则：
1. 素材原文必须完整保留输入原文，不得改写、删减或规范化标点。输入原文已由程序移除末尾“本内容由 Coze AI 生成...”标识行。
...
def clean_lines(lines: list[str]) -> list[str]:
    ...
    while cleaned and cleaned[-1] == "":
        cleaned.pop()
    while cleaned and COZE_FOOTER_RE.match(cleaned[-1]):
        cleaned.pop()
        while cleaned and cleaned[-1] == "":
            cleaned.pop()
    return cleaned
